...
const OfferingCard = ({ index, imageUrl, title, isReversed, subTitle, options, images }) => {
  ...
  return (
    <div
      id={`animatedOfferingCard${index}`}
      className={`flex flex-col justify-between items-center pt-4 pb-4 sm:pt-6 sm:pb-4 md:pt-8 md:pb-8 lg:pt-10 lg:pb-10 xl:pt-16 xl:pb-16 w-full xl:px-60 lg:px-40 sm:px-20 px-10 ${
    ...
    >
      <div
        className={`w-full flex 
          ...
          ${isVisible && !isReversed ? 'animate-slideRight' : ''} 
          ${isVisible && isReversed ? 'animate-slideLeft' : ''} 
          ${index === 4 ? 'w-screen -mr-0 md:-mr-20 lg:-mr-40 xl:-mr-60' : ''}
      ...
      <div className='flex flex-col items-start w-full md:w-3/5 md:px-8'>
        <div
          className={`w-full flex flex-col gap-y-8 ${
            !isVisible ? 'translate-x-full' : '' // Start off-screen to the right if not visible
          } ${
            isVisible && !isReversed ? 'animate-slideLeft' : '' // Slide left if visible and not reversed
          } ${
            isVisible && isReversed ? 'animate-slideRight' : '' // Slide right if visible and reversed
          }`}
        >
          ...
          <div className='flex flex-col justify-between sm:flex-col md:flex-row lg:flex-row xl:flex-row'>
            {
              images.map((image, i) =>
                <div key={i + image.image} className="w-full flex flex-col items-center">
